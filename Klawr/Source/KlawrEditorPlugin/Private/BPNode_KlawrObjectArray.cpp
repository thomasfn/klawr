...
#define LOCTEXT_NAMESPACE "KlawrObjectArrayNode"

/////////////////////////////////////////////////////
// FKCHandler_MakeKlawrObjectArray
class FKCHandler_MakeKlawrObjectArray : public FNodeHandlingFunctor
{
public:
	FKCHandler_MakeKlawrObjectArray(FKismetCompilerContext& InCompilerContext)
	...
	{
		UBPNode_KlawrObjectArray* ArrayNode = CastChecked<UBPNode_KlawrObjectArray>(Node);
...
/////////////////////////////////////////////////////
// UBPNode_KlawrObjectArray

UBPNode_KlawrObjectArray::UBPNode_KlawrObjectArray(const FObjectInitializer& ObjectInitializer)
...
FNodeHandlingFunctor* UBPNode_KlawrObjectArray::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_MakeKlawrObjectArray(CompilerContext);
}

FText UBPNode_KlawrObjectArray::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return LOCTEXT("NodeTitle", "Make Array");
}

UEdGraphPin* UBPNode_KlawrObjectArray::GetOutputPin() const
{
	return FindPin(OutputPinName);
}

void UBPNode_KlawrObjectArray::AllocateDefaultPins()
{
	// Create the output pin
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), NULL, true, false, *OutputPinName);

	// Create the input pins to create the arrays from
	for (int32 i = 0; i < NumInputs; ++i)
	{
		CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, TEXT(""), NULL, false, false, *FString::Printf(TEXT("[%d]"), i));
	}
}

bool UBPNode_KlawrObjectArray::CanResetToWildcard() const
{
...
void UBPNode_KlawrObjectArray::ClearPinTypeToWildcard()
{
...
void UBPNode_KlawrObjectArray::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
void UBPNode_KlawrObjectArray::PropagatePinType()
{
...
UFunction* UBPNode_KlawrObjectArray::GetArrayClearFunction() const
{
...
UFunction* UBPNode_KlawrObjectArray::GetArrayAddFunction() const
{
	UClass* ArrayLibClass = UKismetArrayLibrary::StaticClass();
...
void UBPNode_KlawrObjectArray::PostReconstructNode()
{
...
FText UBPNode_KlawrObjectArray::GetTooltipText() const
{
	return LOCTEXT("MakeArrayTooltip", "Create an array from a series of items.");
}

void UBPNode_KlawrObjectArray::AddInputPin()
{
...
void UBPNode_KlawrObjectArray::RemoveInputPin(UEdGraphPin* Pin)
{
...
void UBPNode_KlawrObjectArray::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if (!Context.bIsDebugging)
	{
		...
		if (Context.Pin != NULL)
		{
			...
			if (Context.Pin->Direction == EGPD_Input)
			{
				...
				Context.MenuBuilder->AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateUObject(this, &UBPNode_KlawrObjectArray::RemoveInputPin, const_cast<UEdGraphPin*>(Context.Pin))
						)
					);
			}
		}
		else
		{
			Context.MenuBuilder->AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateUObject(this, &UBPNode_KlawrObjectArray::AddInputPin)
					)
				);
		}
		...
		Context.MenuBuilder->AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateUObject(this, &UBPNode_KlawrObjectArray::ClearPinTypeToWildcard),
				FCanExecuteAction::CreateUObject(this, &UBPNode_KlawrObjectArray::CanResetToWildcard)
				)
			);
...
bool UBPNode_KlawrObjectArray::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
...
void UBPNode_KlawrObjectArray::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
	auto Schema = Cast<const UEdGraphSchema_K2>(GetSchema());
...
void UBPNode_KlawrObjectArray::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UBPNode_KlawrObjectArray::GetMenuCategory() const
{
